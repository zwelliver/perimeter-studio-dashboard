    ...
    <style>
        ...
        * {{
        ...
        @media (max-width: 768px) {{
            ...
            /* Mobile vertical timeline - day by day layout */
            @media (max-width: 480px) {{
                ...
                .timeline-day-projects {{
                    padding: 8px;
                }}

                .timeline-mobile-project {{
                    display: flex;
                    justify-content: space-between;
                    align-items: center;
                    padding: 10px 12px;
                ...
                .timeline-mobile-project:last-child {{
                    margin-bottom: 0;
                }}

                .timeline-mobile-project.critical {{
                    background: rgba(220, 53, 69, 0.1);
                    border-left-color: #dc3545;
                }}

                .timeline-mobile-project.warning {{
                    background: rgba(255, 193, 7, 0.1);
                    border-left-color: #ffc107;
                }}

                .timeline-mobile-project.normal {{
                    background: rgba(52, 152, 219, 0.1);
                    border-left-color: var(--brand-primary);
                }}

                .timeline-mobile-project-name {{
                    font-size: 13px;
                    font-weight: 500;
                    flex: 1;
                    color: var(--text-primary);
                ...
                .timeline-mobile-project[data-position="start"] .timeline-mobile-project-badge {{
                    background: rgba(40, 167, 69, 0.2);
                    color: #28a745;
                }}

                .timeline-mobile-project[data-position="end"] .timeline-mobile-project-badge {{
...
"""
    ...
    html += f"""
        ...
        function generateTimeline() {{
            ...
            if (isMobile) {{
                // Mobile vertical timeline - day by day
                html = generateMobileTimeline(projects, dates, totalDays);
            }} else {{
                // Desktop horizontal timeline
                html = generateDesktopTimeline(projects, dates, totalDays);
            }}

            timelineContainer.innerHTML = html;
        }}

        function generateDesktopTimeline(projects, dates, totalDays) {{
        ...
        function generateMobileTimeline(projects, dates, totalDays) {{
            let html = '<div class="timeline-mobile">';

            // Create day-centric view
            dates.forEach((date, dayIndex) => {{
                // Find projects active on this day
                const activeProjects = projects.filter(p =>
                    dayIndex >= p.start && dayIndex < (p.start + p.duration)
                );

                html += `
                    <div class="timeline-day-card">
                        <div class="timeline-day-header">
                            <span class="timeline-day-date">${{date}}</span>
                            <span class="timeline-day-count">${{activeProjects.length}} active</span>
                        </div>
                        <div class="timeline-day-projects">
                `;

                if (activeProjects.length === 0) {{
                    html += '<div class="timeline-no-projects">No scheduled items</div>';
                }} else {{
                    activeProjects.forEach(project => {{
                        const isStart = dayIndex === project.start;
                        const isEnd = dayIndex === (project.start + project.duration - 1);
                        const position = isStart ? 'start' : (isEnd ? 'end' : 'middle');

                        html += `
                            <div class="timeline-mobile-project ${{project.status}}" data-position="${{position}}">
                                <span class="timeline-mobile-project-name">${{project.name}}</span>
                                <span class="timeline-mobile-project-badge">
                                    ${{isStart ? '▶ Start' : isEnd ? '⏹ End' : '▬ Active'}}
                                </span>
                            </div>
                        `;
                    }});
        ...
        // Add window resize listener for responsive timeline
